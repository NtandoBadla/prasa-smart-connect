        ...
        <Link to="/" className="flex items-center gap-3">
          ...
          <div className="leading-tight">
            ...
            <div className="text-[10px] uppercase tracking-widest opacity-80">
              AI Powered Smart Commuter System
            </div>
          </div>
        </Link>
        ...
        <nav className="hidden lg:flex items-center gap-1">
          {PRIMARY.map((item) => (
            <Link
              ...
              activeProps={{
                className:
                  "rounded-sm px-3 py-2 text-sm font-medium bg-primary-dark border-b-2 border-destructive",
              }}
              ...
              <div className="absolute right-0 top-full w-56 rounded-md border border-border bg-card p-1 text-foreground shadow-elevated">
                {MORE.map((item) => (
                  <Link
                    ...
                    activeProps={{
                      className:
                        "block rounded-sm px-3 py-2 text-sm bg-secondary font-semibold text-primary",
                    }}
                  >
                    {item.label}
                  </Link>
