...
import { sendEmail, sendSms, notifySubscribers } from "./mailer";
...
async function jobScrapeAndAlert() {
  try {
    ...
    const actionable = trains.filter((t) =>
      ["Delayed", "Cancelled", "No Service"].includes(t.status)
    );
    if (!actionable.length) return;

    // Deduplicate by station to avoid spamming
    const byStation = new Map<string, typeof actionable[0]>();
    for (const t of actionable) {
      if (!byStation.has(t.from_station)) byStation.set(t.from_station, t);
    }

    for (const [station, train] of byStation) {
      ...
      // Email
      const { sent, failed } = await notifySubscribers(subscribers, {
      ...
      // SMS
      const smsText = `PRASA Alert: Train ${train.train_no} (${train.line}) at ${station} — ${
        train.status === "Delayed" ? `Delayed ${train.delay_min} min` : train.status
      }${train.reason ? `. ${train.reason.slice(0, 80)}` : ""}.`;

      await Promise.allSettled(
        subscribers
          .filter((u) => !!u.phone)
          .map((u) => sendSms(u.phone!, smsText).catch(() => {})),
      );

      await auditLog("alert_sent", "scraped_trains", train.train_no, {
        station, status: train.status, emailSent: sent, emailFailed: failed,
      });
    }
  } catch (err: any) {
